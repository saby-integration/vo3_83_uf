...
Процедура СоздатьДополнительноеСвойствоКЭДО()
	...
	СправочникНДРС = "НаборыДополнительныхРеквизитовИСведений"; 
	
	Если Метаданные.Справочники.Найти(СправочникНДРС) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Метаданные.Справочники[СправочникНДРС].Реквизиты.Найти("ИмяПредопределенногоНабора") = Неопределено Тогда 
		Возврат;
	КонецЕсли;	
	...
	Если РазделИнтерфейса = Неопределено Тогда
		РазделИнтерфейса	= Справочники[СправочникНДРС].СоздатьЭлемент();
